<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST')
    {
        ...
        $_SESSION['openedContactID'] = $user->getContact($submVal - 1)->ID;

        $contact = null;

        foreach($user->getContacts() as $potContact)
        {
            if($potContact->ID == $_SESSION['openedContactID'])
            {
                $contact = $potContact;
            }
        }

        header('Location: ../ContactPanel/index.php');
        exit;
    ...
    <div class="container">
        ...
        <div class="user-list">
            ...
            <?php
                ...
                {
                    $counter++;
                    $other = $uDatabase[$contact->getSecondUserID() - 1]->getName() == $user->getName() ? $uDatabase[$contact->getFirstUserID() - 1]->getName() : $uDatabase[$contact->getSecondUserID() - 1]->getName(); 

                    echo "<button type='submit' name='submit' value='" . $counter  . "' class='user-item'>
                            <p><strong>Nazwa kontaktu:</strong> " . $contact->name . "</p>
                            <p><strong>Z:</strong> " . $other . "</p>
                        </button>";
                }
                
                if($counter == 0)
                {
                    echo "<div class='no-users'><p>Brak Kontaktów</p></div>";
                }
        ...
        <a href="../ManageContacts/index.php" class="back-button">Zarządzaj Kontaktami</a>
